...
class DistributedTeleporter(BaseObject):
    ...
    if not IS_CLIENT:
        ...
        def teleporterSend(self, player):
            ...
            self.myNextThink = base.clock.getFrameTime() + 0.1
            self.timesUsed += 1
        ...
        def teleporterReceive(self, player, delay):
            ...
            self.myNextThink = base.clock.getFrameTime() + TELEPORTER_FADEOUT_TIME
            self.timesUsed += 1
        ...
        def onFinishConstruction(self):
            ...
            self.setModel("models/buildables/teleporter_light")
        ...
        def setTeleState(self, state):
            if state != self.teleState:
                ...
                self.lastStateChangeTime = base.clock.getFrameTime()
        ...
        def determinePlaybackRate(self):
            ...
            if self.isBuilding():
                ...
                if self.teleState == TStateReady:
                    ...
                    timeSinceCharge = base.clock.getFrameTime() - self.lastStateChangeTime
                    lowSpinSpeed = 0.15
                    if timeSinceCharge <= 4.0:
                        playRate = TFGlobals.remapVal(base.clock.getFrameTime(),
        ...
        def __teleporterThink(self, task):
            ...
            if self.myNextThink and self.myNextThink > base.clock.getFrameTime():
                return task.again
            ...
            if self.teleState == TStateIdle:
                ...
                if base.clock.getFrameTime() > self.rechargeTime:
                    self.setTeleState(TStateReady)
                    self.emitSoundSpatial("Building_Teleporter.Ready")
                ...
                self.rechargeTime = base.clock.getFrameTime() + (TELEPORTER_FADEOUT_TIME + TELEPORTER_FADEIN_TIME + TELEPORTER_RECHARGE_TIME)
                ...
                self.myNextThink = base.clock.getFrameTime() + TELEPORTER_FADEIN_TIME
                ...
                self.setTeleState(TStateRecharging)
                self.myNextThink = base.clock.getFrameTime() + TELEPORTER_RECHARGE_TIME
        ...
        def __teleUpdate(self, task):
            if self.spinSound:
                ...
                self.spinSound.setPlayRate(self.getPlayRate())

            # Blend in the blur with alpha scale.
            if self.showingBlur:
                if self.currBlurAlpha < 1.0:
                    nodes = self.getBodygroupNodes("teleporter_blur", 1)
                    self.currBlurAlpha = TFGlobals.approach(1.0, self.currBlurAlpha, 1.0 * base.clock.getDt())
                    nodes.setColorScale(1, 1, 1, self.currBlurAlpha)

            elif self.currBlurAlpha > 0.0:
                nodes = self.getBodygroupNodes("teleporter_blur", 1)
                self.currBlurAlpha = TFGlobals.approach(0.0, self.currBlurAlpha, 1.0 * base.clock.getDt())
                nodes.setColorScale(1, 1, 1, self.currBlurAlpha)
