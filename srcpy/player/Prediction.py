...
class PredictionField:
    ...
    def __init__(self, name, type, getter = None, setter = None, private = False, networked = True,
        ...
        self.getter = getter
        if not getter:
            self.getterLambda = eval("lambda x: x.%s" % name)
        self.setter = setter
...
class PredictionCopy:
    ...
    def getValue(self, i, table, tableType, data):
        if tableType == PREDICTION_TABLE_DICT:
            ...
            if data.getter:
                ...
                return data.getterLambda(table)

    def setValue(self, i, fieldName, table, tableType, data, value):
        ...
        if tableType == PREDICTION_TABLE_DICT:
            ...
            if data.setter:
                ...
                setattr(table, fieldName, newVal)
    ...
    def transferData(self, operation, dataDesc, destSlot = -1):
        ...
        for fieldName, data in dataDesc.items():
            ...
            srcValue = self.getValue(x, self.src, self.srcType, data)
            destValue = self.getValue(x, self.dest, self.destType, data)

            difftype = self.getDiffType(destValue, srcValue, data)

            if self.performCopy and difftype != PREDICTION_DIFF_IDENTICAL:
                self.setValue(x, fieldName, self.dest, self.destType, data, srcValue)
